...
function CheckOutCardInformation() {
  ...
  const [messageCheckCardNumber, setMessageCheckCardNumber] = useState("");
  const [messageCheckCardName, setMessageCheckCardName] = useState("");
  const [messageCheckExpiryDate, setMessageCheckExpiryDate] = useState("");
  const [messageCheckSecurityCode, setMessageCheckSecurityCode] = useState("");
  ...
  const handleValidation = async () => {
    setMessageCheckCardNumber("");
    setMessageCheckCardName("");
    setMessageCheckExpiryDate("");
    setMessageCheckSecurityCode("");
    setLoginMessage("");
    if (
      ...
      setMessageCheckCardNumber(
        "Please check the card number, it must have a valid format."
      );
    }
    if (cardName.length < 3) {
      setMessageCheckCardName(
        "Please check the card name, it must have at least 3 characters."
      );
    }
    if (!/^(0[1-9]|1[0-2])\/?([0-9]{4}|[0-9]{2})$/.test(cardExpiryDate)) {
      setMessageCheckExpiryDate(
        "Please check the expiry date, it must have a valid format."
      );
    }
    if (!/^[0-9]{3,4}$/.test(cardSecurityCode)) {
      setMessageCheckSecurityCode(
        "Please check the security code, it must have a valid format."
      );
    }
    if (!user.token) {
          ...
          <Row className="gx-5">
            ...
            <Col md={6} sm={12}>
              ...
              <div className="p-2">
                ...
                <AccordionCards />
                <form onSubmit={(ev) => ev.preventDefault(ev)}>
                  <label
                ...
                <p className="text-danger mt-3" style={{ fontSize: "0.7rem" }}>
                  {messageCheckCardNumber}
                </p>
                <p className="text-danger mt-3" style={{ fontSize: "0.7rem" }}>
                  {messageCheckCardName}
                </p>
                <p className="text-danger mt-3" style={{ fontSize: "0.7rem" }}>
                  {messageCheckExpiryDate}
                </p>
                <p className="text-danger mt-3" style={{ fontSize: "0.7rem" }}>
                  {messageCheckSecurityCode}
                </p>
                <p className="text-danger mt-3" style={{ fontSize: "0.7rem" }}>
                  {messageEmptyCart}
                </p>
...
export default CheckOutCardInformation;
